...
export function ReportProductSales() {
  ...
  const fetchSaleData = async (productName?: string) => {
    try {
      ...
      const response = await axios.get("/api/external/Consultas/ultvendas", {
        params: ultParams,
        headers: {
      ...
      if (error.response?.status === 404) {
        ...
        switch (searchType) {
          case "cnpj":
            message = `Não foram identificadas vendas recentes associadas ao cliente com o CNPJ "${productName}".`;
            break;
          case "codigo":
            message = `Não foram identificadas vendas recentes vinculadas ao código de cliente "${productName}".`;
            break;
          case "nome":
            message = `Não foram identificadas vendas recentes para o cliente denominado "${productName}".`;
            break;
          default:
            message = `Não foram identificadas vendas recentes relacionadas a "${productName}".`;
        }
  ...
  const getEmptyStateMessage = () => {
    ...
    if (searchTerm && saleData.length === 0) return "Realize sua consulta...";
          ...
          <div className="flex flex-col gap-5">
            ...
            <CardTitle className="flex items-center gap-3 sm:gap-4 text-lg sm:text-xl text-slate-900 dark:text-slate-100">
              ...
              <span className="font-semibold text-base sm:text-lg lg:text-xl">
                Relatório Detalhado de Vendas do Cliente
              </span>
            </CardTitle>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <div className="text-xs text-slate-500 dark:text-slate-400">
                                  Código: {sale.codigo} | Estado: {sale.estado}
                                </div>
                              </div>
                            </div>
